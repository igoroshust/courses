## JS
JS - скриптовый язык программирования для создания интерактивных динамических веб-страниц, разработанный компанией Netscape для браузера Netscape Communicator
...
Маршрутизация - процесс передачи пакетов данных между серверами.
...
Движок в среде исполнения браузера работает с HTML и пользовательскими событиями, в серверной среде (NodeJS) - с файловой системой (отвечать за чтение и запись файлов).
...
## ECMA
ECMAScript - спецификация скриптового языка, появившаяся для стандартизации JavaScript (netscape) и JScript (создан microsoft для explorer). Это документ, в котором описаны требования для реализации языка программирования. JavaScript и JScript стали диалектами ECMASCript - разновидностями одного языка с разной реализацией.

ECMA262 - спецификация международной организации ECMA International, которая появилась для стандартизации JS и JScript.

## NodeJS
Среда исполнения JS вне браузера посредством использования того же движка, что и в Google Chrome, V8. NodeJS содержит интерпретатор и позволяет переводить его в машинный код.

**Версии NodeJS:**
- LTS (long term support) - стабильная, стандартная версия для распространения (без дефектов и скрытного поведения). Все предоставляемые ей функции будут протестированы и задокументированы.

- Current - последняя версия от разработчиков NodeJS. В ней доступны тестовые функции, но не гарантии в их стабильности.


### Компиляторы

Минус: не кроссплатформенный

![alt text](image-7.png)

### Разное
- Транслирование - перевод высокоуровневых ЯП в машинный код
- Высокоуровневые языки - языки, близкие и понятные человеку по семантике
- Машинный код - последовательность 0 и 1