...
modify_global()  # inside x = modif glob
print(x)  # modif glob
```
...
Идиомы в программировании - устоявшийся, узнаваемый способ решать типичную задачу на конкретном языке. Это не отедельный инструмент, а паттерн использования языка: набор приёмов, который сообщество программистов считает правильным или каноническим для определённой ситуации.
...
- Множества - неупорядоченные коллекции уникальных элементов. Только уникальные элементы
...
Множественная вставка в список через срез
...
Словарь (dict) - это коллекция пар "ключ-значение". Обращение к значению происходит по ключу, а не по числовой позиции. Словарь убирает посредника-индекс и связывает имя со значением напрямую. Тип организации данных по типу телефонной книги: ищем человека по имени, а не по номеру строки.

```Python
grades = {'John': 90, 'Mary': 75, 'Kate': 80}
print(grades["Mary"])  # 75
```

Преобразование готовых пар в словарь

```Python
pairs = [("name", "Anna"), ("age", 28), ("city", "Berlin")]
person = dict(pairs)
print(person) # {'name': 'Anna', 'age': 28, 'city': 'Berlin'}
```

Обращение через brackets `person["age"]` при несуществующем ключе вызывает ошибку, обращение через dict.get(key) – нет (вернёт None или значение по умолчанию).

```Python
grades = {'John': 90, 'Mary': 75, 'Kate': 80}

print(
  	grages['Joaz'],  # KeyError
  	grades.get('Jogns'),  # None
    grades.get('Johns', 'не указан')  # не указан
)
```

Правило выбора: скобки - когда отсутствие ключа это ошибка, get() - когда отсутствие это нормальный случай, который вы готовы обработать.

### Добавление и изменение

У словаря один и тот же синтаксис добавляет и изменяет существующий ключ. Если ключа не было - он появится, если был - значение перезапишется.

```Python
person = {"name": "John", "age": 30}

person["city"] = "New York"  # Добавление нового
person["age"] = 31  # Перезапись
```

Когда необходимо внести несколько значений, используется метод `update()`

```Python
grades = {'John': 90, 'Mary': 75, 'Kate': 80}

grades.update({'John': 91, 'Mary': 80})
grades.update([('John', 91), ('Mary', 80)])
```

Проверка наличия ключа происходит благодаря `in`. Не вызывает ошибки
...
print(
    'John' in grades,  # True
    'Joahnn' in grades,  # False
)
```

Удаление ключа через `del`, падает с ошибкой.

Метод `pop()` удаляет ключ и заодно возвращает его значение - полезно, когда удаляемое значение ещё нужно. Со вторым аргументом он не падает на отсутствующем ключе, а возвращает значение по умолчанию.
...
- Уникальность. Два одинаковых ключа быть не может.
- Неизменяемость. Подходят строки, числа, кортежи (списки не могут быть ключами). Python находит значение по ключу через его неизменяемое содержимое.
